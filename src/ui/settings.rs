    ...
    egui::Window::new("Settings")
        ...
        .show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                // General
                egui::CollapsingHeader::new(RichText::new("⚙ General").size(15.0).strong())
                    .default_open(true)
                    .show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ui.label("Theme:");
                ...
                // Performance
                egui::CollapsingHeader::new(RichText::new("⚡ Performance").size(15.0).strong())
                    .default_open(false)
                    .show(ui, |ui| {
                        ui.horizontal(|ui| {
                ...
                // Context Menu
                egui::CollapsingHeader::new(RichText::new("📂 Context Menu").size(15.0).strong())
                    .default_open(false)
                    .show(ui, |ui| {
                            ...
                            if !is_registered {
                                if widgets::accent_button(ui, "Register").clicked() {
                                    match context_menu::register_context_menu() {
                                        ...
                                        Err(e) => {
                                            app.status_message = format!("Failed to register: {e}")
                                        }
                                    }
                                }
                                ...
                                match context_menu::unregister_context_menu() {
                                    Ok(_) => {
                                        app.status_message = "Context menu unregistered".to_string()
                                    }
                                    Err(e) => {
                                        app.status_message = format!("Failed to unregister: {e}")
                                    }
                                }
                            }
                        });
                ...
                // About
                egui::CollapsingHeader::new(RichText::new("ℹ About").size(15.0).strong())
                    .default_open(false)
                    .show(ui, |ui| {
                        ui.horizontal(|ui| {
